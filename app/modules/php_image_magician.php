...
## --------------------------------------------------------
  ...
  # Date:     27-02-08
    ...
    $this->fileExtension = fix_strtolower_(strrchr($fileName, '.'));
...
*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-**-*-*-*-*-*-*-*-*-*-*-*-*-*/
  ...
  #       forceStretch can be applied to options 1,2,3 and 4
    ...
    $cropPos = 'm';
    if (is_array($option) && fix_strtolower_($option[0]) == 'crop') {
...
## --------------------------------------------------------
  ...
  {
    if (is_array($option)) {
      if (fix_strtolower_($option[0]) == 'crop' && count($option) == 2) {
        return 'crop';
      } else {
...
*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-**-*-*-*-*-*-*-*-*-*-*-*-*-*/
    ...
    #
  {
    ...
    $isTransparent = false;
    if (!is_array($bgColor)) {
      if (fix_strtolower_($bgColor) == 'transparent') {
        $isTransparent = true;
      }
    }
...
*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-**-*-*-*-*-*-*-*-*-*-*-*-*-*/
    ...
    #
  {
    ...
    if (fix_strtolower_(strrchr($watermarkImage, '.')) == '.png') {
...
## --------------------------------------------------------
    ...
    if (!file_exists($file) && !$this->checkStringStartsWith('http://', $file)) { if ($this->debug) { throw new Exception('Image not found.'); }else{ throw new Exception(); }};
        ...
        $extension = fix_strtolower_($extension);
        switch($extension)
        {
            case '.jpg':
            case '.jpeg':
...
## --------------------------------------------------------
    ...
    #
  {
    ...
    } else if (fix_strtolower_($value) == 'transparent') {
